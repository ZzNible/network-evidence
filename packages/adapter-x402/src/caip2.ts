/**
 * CAIP-2 parsing for the x402 `network` field, restricted to the EVM family
 ...
 * `X402_NETWORK_FAMILY_UNSUPPORTED`; malformed identifiers fail with
 * `X402_NETWORK_MALFORMED`.
 */
export function parseCaip2EvmNetwork(value: unknown): Caip2EvmNetwork {
  if (typeof value !== "string") {
    x402Fail("X402_NETWORK_MALFORMED", "network must be a CAIP-2 string");
  }
  ...
  if (!/^[0-9]+$/.test(reference)) {
    x402Fail(
      "X402_NETWORK_MALFORMED",
      "eip155 reference must be a decimal chain id",
    );
  }
